...
const CategoryWiseArticles = () => {
  ...
  const [politicsArticle, setPoliticsArticle] = useState([
    {}, {}, {}, {}
  ])
  return (
    <div className="category-wise-articles w-full h-auto pl-3 lg:pl-10 lg:px-10 pb-10">
      {navItems.map((cat) => (
        <div className="pt-6">
          <h2 className="font-RobotoSlab text-[32px] lg:text-[34px] font-bold">{cat.name}</h2>
          <div className="flex flex-row items-center space-x-7 overflow-x-scroll mt-4 border-b pb-8">
            {politicsArticle.map((article) => (
              <div className='article-item min-w-[220px] lg:min-w-0 lg:flex-1 h-72 flex flex-col cursor-pointer'>
                <div className="article-item-img-area h-3/4 w-full relative">
                  <Image
                ...
                <h3 className="hero-posts-small-sub-heading font-RobotoSlab text-lg mt-4 hover:text-gray-500 font-medium">
          ...
          <div className="w-full flex flex-row border-b">
            <h5 className="font-RobotoSlab text-sm py-1 ml-auto cursor-pointer hover:text-gray-500 mr-3 lg:mr-0">More {cat.name} {">"}</h5>
          </div>
        </div>
      ))}
    </div>
